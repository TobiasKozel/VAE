...
namespace vae { namespace core {
	struct Source {
		SourceHandle id;
		bool stream  _VAE_SMALL(1);		///< If false entire sample will be loaded in ram, there's no streaming for now
		bool resample _VAE_SMALL(1);	///< Whether the sound will be resampled when loading it
		bool raw _VAE_SMALL(1);			///< The data in path is a data buffer
		enum class Format {
